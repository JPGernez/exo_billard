package com.example.exo_billard;

import android.graphics.Color;

public class Constantes {

	// rayon de representaion des billes
	public static final float rayon = 2.41f;
	// rayon de selection lors d un click a l ecran
	public static final float rayonSelect = 4f;

	//Couleur utilisée
	// tapis vert
	public static final int couleurBandeG = Color.rgb(58, 12, 7);
	public static final int couleurTapisG = Color.rgb(14, 47, 20);
	public static final int couleurRadarG = Color.rgb(135, 233, 144);
	public static final int couleurMoucheG = Color.rgb(56, 111, 72);
	public static final int couleurLigneG = Color.rgb(9, 82, 40);
	public static final int couleurBackG = Color.rgb(113, 111, 87);
	public static final int couleurSelG = Color.rgb(121, 128, 129);

	//tapis bleu
	public static final int couleurBandeB = Color.rgb(3, 24, 66);
	public static final int couleurTapisB=Color.rgb(59, 134,134);
	public static final int couleurRadarB = Color.rgb(169, 219, 169);
	public static final int couleurMoucheB = Color.rgb(116, 208, 241);
	public static final int couleurLigneB = Color.rgb(121, 189, 154);
	public static final int couleurBackB = Color.rgb(29, 72, 101);
	public static final int couleurSelB = Color.rgb(121, 128, 129);

	//tapis rouge
	public static final int couleurBandeR = Color.rgb(50, 13, 13);
	public static final int couleurTapisR=Color.rgb(109, 7, 26);
	public static final int couleurRadarR = Color.rgb(254, 150, 160);
	public static final int couleurMoucheR = Color.rgb(254, 250, 250);
	public static final int couleurLigneR = Color.rgb(145, 40, 59);
	public static final int couleurBackR = Color.rgb(174, 74, 52);
	public static final int couleurSelR = Color.rgb(121, 128, 129);

	//tapis N&B
	public static final int couleurBandeNB = Color.rgb(121, 128, 129);
	public static final int couleurTapisNB = Color.rgb(48, 48, 48);
	public static final int couleurRadarNB = Color.rgb(158, 158, 158);
	public static final int couleurMoucheNB = Color.rgb(206, 206, 206);
	public static final int couleurLigneNB = Color.rgb(11, 22, 22);
	public static final int couleurBackNB = Color.rgb(20, 20, 20);
	public static final int couleurSelNB = Color.rgb(121, 128, 129);


	//billes
	public static final int couleurBJaune=Color.rgb(255, 215, 0);
	public static final int couleurBRouge=Color.rgb(254, 27, 0);
	public static final int couleurBBlanche=Color.rgb(253, 233, 224);

	public static final int couleurBack = Color.rgb(0, 0, 0);


}
